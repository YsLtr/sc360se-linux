...
| Sleep timeout + move wakeup (0x07) | ✅ | 休眠设置.pcapng |
...
- **Buttons section** — 6 rows, each with a type dropdown that swaps
  ...
  widgets
- **Other** — polling rate (125/250/500/1000 Hz), sleep timeout, move wakeup
...
sc360se sleep 900 1                        # 15 min stage-2 sleep, move wake enabled
sc360se move-wakeup 0                      # keep current sleep timeout, click wake only
...
| 0x06 | 0x05 | sensor advanced | Not exposed; previous light-mode naming was wrong |
| 0x07 | 0x04 | power management | [4]=stage-2 sleep in 10s units; [5]=move_wakeup; [6]=kept 0; [7]=button_respondtime default 8 |
...
| 0x16 | — | sensor-advanced readback | query: bare op; not exposed in high-level UI |
| 0x17 | — | power readback | query: bare op; reply sub=0x05, [4]=sleep_units, [5]=move_wakeup |
...
The normal Linux CLI/GUI intentionally exposes only controls with useful
SC360SE evidence. Do not add UI for these without a dedicated capture and live
behavior check:

- full LED effect editing
- LED brightness
- LOD (lift-off distance) UI support for this model
- Button debounce
- Sub-100 cpi DPI granularity (or sub-500 above 5000 cpi)

The PAW3395 sensor itself supports 50 cpi steps and configurable LOD, and
static reversing maps `0x06/0x05` to a hidden sensor-advanced group. SC360SE's
official XML hides LOD/sensor controls, so these are not exposed as supported
features here. To add unsupported features you would need to either:
...
3. **Software workarounds** — debounce can be done in libinput via a
   per-device quirk; LOD cannot be made equivalent in software.

The honest summary: **a Linux driver for this device should expose only the
SC360SE controls that captures or live tests prove useful.** This driver
matches the verified Windows-driver configuration subset; raw `send` / `recv`
/ `monitor` remain available for future protocol experiments without adding
untested features to the normal CLI or GUI.
